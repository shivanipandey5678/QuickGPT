...
const Sidebar = ({isMenuOpen, setIsMenuOpen}) => {
  ...
  return (
    ...
    >
      ...
      {/* new chat button */}
      <button className="flex justify-center items-center w-full py-2 mt-7 text-white bg-gradient-to-r from-[#A456f7] to-[#3D81F6] text-sm rounded-md cursor-pointer">
      ...
      <div className="mt-5">
        <div onClick={()=>{navigate('/community'); setIsMenuOpen(false)}} className="flex items-center gap-4 p-3 mt-2 border border-gray-400 dark:border-white/20 rounded-md hover:scale-99 transition-all delay-75 cursor-pointer">
          <img
        ...
        <div onClick={()=>{navigate('/credits'); setIsMenuOpen(false)}} className="flex items-center gap-4 p-2 mt-2  border border-gray-400 dark:border-white/20 rounded-md hover:scale-99 transition-all delay-75 cursor-pointer">
          <img src={assets.diamond_icon} alt="search_icon" className="w-4 dark:invert  " />
          <div>
            <p className="font-medium">Credits : 200</p>
            <p className="text-xs text-gray-500 dark:text-white">
        ...
        <div className="flex items-center gap-4 p-3 mt-2 border border-gray-400 dark:border-white/20 rounded-md cursor-pointer group">
          <img src={assets.user_icon} alt="search_icon" className="w-7 rounded-full  " />
          <p className="flex-1 text-sm dark:text-primary truncate">{user ? user.name : 'Login your account'}</p>
          {user && <img src={assets.logout_icon} className="hidden group-hover:block w-5 h-5  not-dark:invert md:hidden cursor-pointer"/>}
