        ...
        {images.length>0 ? (
           <div className='flex flex-wrap max-sm justify-center gap-5 '>
               {images.map((item,index)=>(
                <a key={index} href={item.imageUrl} target='_blank'  className='relative group block rounded-lg overflow-hidden border border-white/40 shadow-sm hover:shadow-md transition-shadow duration-300'>
                  <img src={item.imageUrl} alt=""  className='w-full h-40 md:h-50 2xl:h-62 group-hover:scale-105 transition-transform duration-300 ease-in-out'/>
